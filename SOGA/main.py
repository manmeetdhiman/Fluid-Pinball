...
population = cost_calc(gen = gen,population = population)
...
while gen <= max_gen:
    ...
    if cost_fittest <= target_cost:
        break
    gen += 1
    children = mate(parents = parents, mut_prob = mut_prob, mut_type = mut_type, g = gen, G = max_gen, limit = search_limit)
    #children = mate_ras(parents = parents, mut_prob = mut_prob, mut_type = mut_type, g = gen, G = max_gen, limit = search_limit)
