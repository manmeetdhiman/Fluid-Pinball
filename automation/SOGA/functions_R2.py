...
def GA(starting_gen, target_cost, max_gen, size, mut_prob, mut_type, search_limit, dt, tsteps,n_genes,abs_counter):
    ...
    if GA_type == 'Pinball':
        ...
        if starting_gen == 0:
            ...
            output_saturation_parameters(gen=starting_gen, sat_counter=sat_counter)
        else:
            population = load_population(starting_gen)
            sat_counter = load_saturation_parameters(starting_gen)
            cost_fittest_s = load_cost_fittest()
            cost_population = load_cost_population()
        ...
        for gen in range(starting_gen+1,max_gen+1):
            ...
            output_saturation_parameters(gen=gen, sat_counter=sat_counter)
            output_cost_population(cost_population=cost_population)
...
def output_saturation_parameters(gen, sat_counter):
    output = {}
...
def output_cost_population(cost_population):
    with open(f'cost_population.yaml', 'w') as f:
        yaml.dump(cost_population, f)


def load_cost_population():
    with open(f'cost_population.yaml', 'r') as f:
        cost_population = yaml.safe_load(f)
